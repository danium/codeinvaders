...
const codexWindowsWrapper = String.raw`@echo off
...
async function buildWindowsWrapper(output) {
  const outputPath = join(root, output);
  await fs.mkdir(dirname(outputPath), { recursive: true });
  await fs.writeFile(outputPath, codexWindowsWrapper, 'utf8');
  return {
    adapter: 'codex-windows-wrapper',
    output,
    bytes: Buffer.byteLength(codexWindowsWrapper),
  };
}
